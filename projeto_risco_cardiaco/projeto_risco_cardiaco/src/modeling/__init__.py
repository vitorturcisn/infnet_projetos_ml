"""
modeling/ — Módulo de Experimentação e Modelagem MLOps (Risco Cardíaco).

API pública:
  • ModelingStep       — etapa PipelineStep completa (orquestra toda a modelagem)
  • FeatureReducer     — redutor de features sklearn-compatível (none|pca)
...
Submódulos internos:
  ...
  cross_validation — CVRunner (KFold estratificado leak-free)
  optimizer        — OptimizerFactory + OptunaOptimizer
  ensemble         — EnsembleBuilder (Voting)
  evaluator        — HoldoutEvaluator
  artifacts        — ArtifactGenerator (plots de classificação: ROC, Confusão)
