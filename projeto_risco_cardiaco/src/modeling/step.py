...
class ModelingStep(PipelineStep):
    ...
    def run(self) -> None:
        ...
        tracker = MLflowTracker(
            ...
            experiment_name=self._cfg.get('modeling', {}).get('experiment_name', 'cardiac-risk'),
            root_dir=self.context.root_dir,
            logger=self.logger,
        )
        otimizador = OptimizerFactory.criar(self._cfg, cv_runner, self._cfg.get('pipeline', {}), self._cfg.get('modeling', {}).get('random_seed', 42), self.logger)
        gerador_artefatos = ArtifactGenerator(self.context.root_dir / self._cfg.get('artifacts', {}).get('output_dir', 'outputs/modeling'), self._cfg.get('artifacts', {}), self.logger)
        ...
        best_run_id = tracker.logar_melhor_modelo(
            model_name=nome_melhor, model=melhor_modelo, best_params=resultado_melhor['best_params'],
            reducer_params=_params_reducer_padrao(self._cfg.get('feature_reduction', {})),
            cv_metrics={'cv_roc_auc_mean': resultado_melhor['cv_roc_auc_mean']},
            train_metrics=metricas_treino, fold_metrics=resultado_melhor['fold_metrics'],
            plot_paths=plot_paths, tuned=resultado_melhor.get('tuned', False)
        )
        ...
        tracker.registrar_modelo(best_run_id, self._cfg.get('modeling', {}).get('registry_name', 'cardiac-best'))
    ...
    def _dividir_treino_holdout(self, X, y):
        ...
        # Classificação: stratify=y para manter proporção de doentes (20%)
        X_train, X_holdout, y_train, y_holdout = train_test_split(X, y, test_size=test_size, random_state=42, stratify=y)
    ...
    def _executar_baseline(self, cv_runner, tracker, X_train, y_train):
        ...
        for nome_modelo, cfg_modelo in self._cfg.get('models', {}).items():
            ...
            resultados[nome_modelo] = {**agg, 'fold_metrics': fold_mets, 'model_cfg': cfg_modelo, 'best_params': dict(cfg_modelo.get('default_params') or {}), 'tuned': False}
    ...
    def _executar_otimizacao(self, otimizador, tracker, cv_runner, resultados, X_train, y_train):
        ...
        for nome_modelo, cfg_modelo in self._cfg.get('models', {}).items():
            ...
            with tracker.contexto_otimizacao(nome_modelo):
                res_opt = otimizador.otimizar(nome_modelo, cfg_modelo, X_train, y_train, self._cfg.get('pipeline', {}), self._cfg.get('feature_reduction', {}))
            
            if not res_opt: continue
            
            pipeline_otimizado = construir_pipeline(cfg_modelo, res_opt['estimator_params'], None, self._cfg.get('pipeline', {}))
            fold_mets = cv_runner.executar(pipeline_otimizado, X_train, y_train)
            agg = agregar_metricas_folds(fold_mets)
            
            # MAIOR é melhor para AUC
            if agg['cv_roc_auc_mean'] > resultados[nome_modelo]['cv_roc_auc_mean']:
                resultados[nome_modelo].update({**agg, 'fold_metrics': fold_mets, 'best_params': res_opt['estimator_params'], 'tuned': True})
                self.logger.info('    ✔ Melhorou para AUC: %.4f', agg['cv_roc_auc_mean'])
        return resultados

    def _executar_ensembles(self, resultados, cv_runner, X_train, y_train):
        builder = EnsembleBuilder(self._cfg.get('ensembles', {}), cv_runner, self._cfg.get('pipeline', {}), self._cfg.get('feature_reduction', {}), logger=self.logger)
        top_entries = sorted(resultados.items(), key=lambda x: x[1]['cv_roc_auc_mean'], reverse=True)[:self._cfg.get('ensembles', {}).get('top_n_base_models', 2)]
    ...
    def _treinar_melhor_modelo(self, nome, resultado):
        ...
        return construir_pipeline(resultado['model_cfg'], resultado['best_params'], None, self._cfg.get('pipeline', {}))